...
j <- 1
while(i <= rnum){
  if(csv_main[i,"TR_OM_POR_RXN_ISOBUTANOLtex"] > 0.8*csv_main[rnum+1,"TR_OM_POR_RXN_ISOBUTANOLtex"] ){  # selecting the ones above the threshold considered threshold in 80 percent of max isobutanol and above  If the name of the target flux reaction is changed you can get the results for that metabolite.
    k <- 1
    while(k < cnum){
...
write.csv(dataf,"maxbutanol.csv") # creates a csv files enlisting the entries amongst the samples producing maximate isobutanol.

x <- read.csv("Ecoli_isobutanol_posybal_all.csv")
y <- x[-1]
...
write.table(q,row.names=FALSE,col.names=FALSE,quote=FALSE,"reactions_isobutanol.txt") # This shall dump a file enlisting the reactions that are running low for maximising current target reaction flux
